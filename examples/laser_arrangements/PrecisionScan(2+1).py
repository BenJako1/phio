...
io = Laser()
io.set_pulsed(550e-9, 1e-5, 25e-9, 150)
io.geometry.set_focused_beam(5.5e-6, 190e-3, 10e-3)

ex = Laser()
ex.set_pulsed(249.6e-9, 1e-5, 25e-9, 150)
...
ax.plot(t, N[2], 'k-', label="ion")
ax.plot(t, N[1], 'k:', label="ex")
ax.plot(t, N[0], 'k--', label="base")
ax.grid()
ax.set_xlim(t[0], t[-1])
ax.set_ylim(np.min(N), np.max(N))
ax.set_xlabel("Time [s]")
ax.set_ylabel("Population")
ax.set_title("Population change over pulse")
plt.show()
'''
num_rep = 5
